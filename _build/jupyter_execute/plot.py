...
# 
# `matplotlib` 패키지는 파이썬 표준 시각화 도구라도 불릴 정도 널리 활용되고 있다. 그래프 요소를 세세하게 꾸밀 수 있을 뿐만 아니라 배우는것도 그리 어렵지 않다. 먼저 해당 패키지의 기본적인 사용법에 대해 알아보도록 하자.
...
# 히스토그램으로 표현하고 싶은 열만 선택한 후, `plot.hist()` 메서드를 적용하면 그래프가 표현된다. matplotlib 패키지를 사용했을 때가 동일하게 bins 인자를 통해 몇개의 구간으로 나눌지도 선택할 수 있다.
# 
...
# `heatmap()` 함수를 통해 히트맵을 나타낼 수 있으며, annot은 데이터의 값을 표시할 지 여부를 의미한다. 또한 cmap은 팔레트 종류로써 값이 높을 수록 붉은색, 낮을 수록 푸른색을 표현한다. class가 높을 수록 생존율이 높고, 남성에 비해 여성의 생존율이 높다는 것을 한 눈에 확인할 수 있다.
# 
# ### 한 번에 여러개의 그래프 나타내기
# 
