...
# ```
# 
# 마지막으로 {numref}`install_talib`와 같이 프롬프트에서 다운로드 받은 파일이 있는 폴더로 디렉토리를 변경한 후, 아래와 같이 설치한다. (폴더 내에 다운로드 받은 파일이 있어야 한다.)
...
# 1. talib 패키지의 `SMA()` 함수를 이용하면 손쉽게 단순 이동평균을 계산할 수 있다. 주가에 해당하는 열을 입력한 후, `timeperiod`에는 얼마의 기간에 해당하는 평균을 계산할지를 입력한다.
...
# 
# RSI 신호에 따라 매매하는 것이 맞을때도 있지만 그렇지 않을때도 있다.
# 
# ## 볼린저밴드
# 
