...
# ## 데이터 불러오기 및 저장하기
# 
# 데이터 분석에서 가장 첫 단계는 외부에 저장된 데이터를 프로그램으로 불러오는 일이다. 데이터가 없다면 분석도 할 수 없기 때문이다. 판다스의 함수를 사용하면 다양한 형태의 파일을 불러와 데이터프레임으로 변환할 수 있으며, 반대로 가공한 데이터프레임을 다양한 유형의 파일로 저장할 수도 있다.
# 
...
# 
# ```{notice}
# 판다스는 넘파이(NumPy)를 기반으로 만들어진 패키지이므로 NumPy에서 사용하는 자료형을 사용한다. 이는 파이썬 기본 자료형과 비슷하지만 다소 차이가 있다.
# 
# | 파이썬 기본 자료형 | 판다스 자료형 | 설명 |
# | --- | --- | --- |
# | int | int64 | 정수형 |
# | float | float64 | 실수형 |
# | string | object | 문자열 |
...
# ```
# 
# 각 열의 고유값 개수를 구하는데는 `value_counts()` 메소드가 사용된다. 먼저 sex(성별)의 고유값의 종류와 개수를 확인해보도록 하자.
...
# 
# 판다스의 메서드를 이용하면 각종 통계값도 쉽게 구할 수 있다. 가장 많이 사용되는 통계값인 산술평균은 `mean()` 메서드를 통해 구할 수 있다.
...
# ## 데이터 재구조화
# 
# 데이터프레임의 행과 열 구조를 변형하거나, 특정 요인에 따라 집계를 하는 방법에 대해 알아보겠다. 판다스에서 데이터 재구조화에 사용되는 함수는 `melt()`, `pivot_table()`, `stack()`, `unstack()` 등이 있다. 예제로써 seaborn 패키지의 팽귄 데이터를 사용한다.
...
# ## 데이터프레임에 함수 적용하기
# 
# `apply()` 메서드를 사용하면 시리즈나 데이터프레임의 개별 원소에 함수를 적용할 수 있다. 이는 사용자가 직접 만든 함수(lambda 함수 포함)를 적용할 수 있어 기본함수로 처리하기 어려운 복잡한 연산도 가능하며, `for` 문을 사용해 각 데이터에 함수를 적용하는 것 보다 더 빠르게 연산이 가능하다.
# 
# ### 시리즈에 함수 적용하기
# 
...
# `apply(max)` 혹은 `apply(max, axis=0)`를 입력하면 각 열 별로 최대값을 구한다. 위 예에서는 bill_length_mm 열의 최대값인 59.6이 계산된다. 이번에는 각 행에 함수를 적용해보자.
...
# `groupby()` 함수 내에 기준이 되는 열을 입력하면 그룹 객체가 만들어진다. 현재는 분할만 이루어진 상태이므로 데이터를 출력해도 기존의 데이터프레임과는 크게 차이가 나지 않는다.
...
# 그러나 `for` 문을 이용해 그룹객체의 이름과 데이터를 확인해보면 species인 Adelie, Chinstrap, Gentoo에 따라 데이터가 분할되어 있다.
# 
# ### 그룹 별 연산하기
# 
...
# `info()` 메서드를 통해 확인해보면 해당 열의 타입이 object 즉 문자열이다. 판다스에서는 `to_datetime()` 메서드를 통해 문자열을 datetime 객체로 변환할 수 있다.
...
# ### 시계열 데이터 만들기
# 
# 앞서 `range()` 함수를 통해 숫자(정수) 리스트를 만들었듯이, 판다스의 `date_range()` 함수를 통해 여러 개의 날짜가 들어있는 배열 형태의 시계열 데이터를 만들 수 있다. 예제로 2021년 1월부터 2021년 12월까지 한 달 간격으로 시계열 데이터를 만들어보자.
