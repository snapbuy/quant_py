...
# 위 코드는 0부터 2까지 `for` 문 내에서 i와 현재 시간을 출력하며, `time.sleep(2)`을 통해 루프당 2초의 일시정지를 준다. 출력된 결과를 살펴보면 한 번의 루프가 끝난 후 약 2초 후에 다음 루프가 실행된 것을 확인할 수 있다.
# 
# ## 제어문
# 
...
# ```
# 
# 프롬프트에서 `pip install 패키지명`을 입력하면 패키지를 설치한다. bt 패키지를 설치해보도록 하자.
